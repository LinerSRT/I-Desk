...
public class SmileRatingBar extends View implements TouchActiveIndicator {
    ...
    public SmileRatingBar(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SmileRatingBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        mSlideAnimator.setDuration(200);
        mSlideAnimator.setInterpolator(new OvershootInterpolator());
        ...
        mAppearAnimator.setInterpolator(new OvershootInterpolator());
        ...
        mAppearAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mSmileyAppearScale == 0) {
                    mSelectedSmiley = SmileyRating.Type.NONE;
                }
                if (mOnSmileySelectedListener != null) {
                    mOnSmileySelectedListener.onSmileySelected(mSelectedSmiley);
                }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mPlaceHolders[0] != null) {
            ...
            for (int i = 0; i < mPlaceHolderPaths.length; i++) {
                ...
                if (i == mCurrentFocusedIndex && SmileyRating.Type.NONE != mSelectedSmiley) {
            ...
            drawSmileyInRect(canvas, mFacePosition, mSmileyPath,
                    ...
                    drawColor, faceColor, SmileyRating.Type.NONE != mSelectedSmiley);
        }
    }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isActiveSmiley(x, y)) {
                    ...
                    if (SmileyRating.Type.NONE == mSelectedSmiley) {
                        mActiveFaceClicked = true;
    ...
    private void animateAppearance(int index) {
        SmileyRating.Type type = SmileyRating.Type.values()[index];
    ...
    private void moveFaceToNearestPlace() {
        ...
        mSelectedSmiley = SmileyRating.Type.values()[index];
    ...
    private void animateSmileyTo(float x, float y) {
        for (int i = 0; i < mPlaceHolders.length; i++) {
            RectF holder = mPlaceHolders[i];
            if (inFaceBounds(x, y, holder)) {
                mSelectedSmiley = SmileyRating.Type.values()[i];
    ...
    public void setRating(SmileyRating.Type type) {
        setRating(type, false);
    }

    public void setRating(SmileyRating.Type type, boolean animate) {
    ...
    public void setRating(int rating, boolean animate) {
        ...
        mSelectedSmiley = SmileyRating.Type.values()[rating - 1];
        if (mInflationDone) {
            if (!animate) {
    ...
    public void setTitle(SmileyRating.Type type, String title) {
        if (SmileyRating.Type.NONE == type) {
            return;
        }
    ...
    public void setFaceBackgroundColor(SmileyRating.Type type, int color) {
        if (SmileyRating.Type.NONE == type) {
            return;
        }
    ...
    public void setFaceColor(SmileyRating.Type type, int color) {
        if (SmileyRating.Type.NONE == type) {
            return;
        }
